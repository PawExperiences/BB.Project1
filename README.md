...
| `input.js`       | Player & Input                       | not yet created |
| `player.js`      | Player & Input                       | not yet created |
...
4. Press **ENTER**. Confirm the screen switches immediately (no page
   ...
   top-right. There is no player ship or invaders yet -- that's expected,
   as this card only provides the scaffold.
...
6. To manually reach the `GameOver` scene (since no lives-loss trigger
...
7. Press **ENTER** again. Confirm it returns immediately to the `Title`
