...
- **pip dependencies** — install from [`requirements.txt`](requirements.txt):
  ...
  Current dependencies include `numpy`, `pandas`, `scikit-learn`, `matplotlib`, and `boto3` (see `requirements.txt` for pinned minimum versions).

---

## Configuration

The profiling script connects to a third-party, read-only PostgreSQL database via the environment variable **`BB_SOURCE_DSN`**.
